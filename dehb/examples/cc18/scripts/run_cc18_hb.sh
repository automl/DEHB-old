...
PYTHONPATH=$PWD python3 dehb/examples/cc18/run_hyperband_cc18.py --verbose True --n_iters $iters --run_id $SLURM_ARRAY_TASK_ID --task_id 189906 \
        --output_path ~/DE_NAS/DEHB/dehb/examples/cc18/results/

if [ 1000 -eq $SLURM_ARRAY_TASK_ID ]; then
    PYTHONPATH=$PWD python3 dehb/examples/cc18/run_hyperband_cc18.py \
        --n_iters $iters --runs $runs --task_id 126031 --verbose True \
        --output_path ~/DE_NAS/DEHB/dehb/examples/cc18/results/
fi

if [ 2000 -eq $SLURM_ARRAY_TASK_ID ]; then
    PYTHONPATH=$PWD python3 dehb/examples/cc18/run_hyperband_cc18.py --verbose True \
